...
def torch_ncut_detection(proposals, sim_matrix=None, original_labels=None, thresh=0.1, num_cuts=10, device=torch.device('cpu')):
    w = pairwise_function(proposals, proposals, device).clone().detach().to(torch.float64)
    w = w * torch.nn.Sigmoid()(sim_matrix)

    node_index   = torch.arange(w.shape[0], dtype=torch.int64, device=device)
    if original_labels == None:
        original_labels = torch.arange(w.shape[0], dtype=torch.int64, device=device)
