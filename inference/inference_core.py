...
class ProbabilisticPredictor(ABC):
    ...
    def __call__(self, input_im, pretest=False):
        ...
        OOD_results = inference_utils.probabilistic_detector_postprocess(OOD_results,
                                                                         input_im[0].get("height",
                                                                                         OOD_results.image_size[0]),
                                                                         input_im[0].get("width",
                                                                                         OOD_results.image_size[1]))
    ...
    def visualize_inference(self, inputs, results, savedir, name, cfg, energy_threshold=None):
        ...
        if energy_threshold:
            labels[(np.argwhere(
                torch.logsumexp(inter_feat[:, :-1], dim=1).cpu().data.numpy() < energy_threshold)).reshape(-1)] = len(
                v_pred.metadata.get("thing_classes", None)) - 1
        # # if name == '133631':
        #     # breakpoint()
        # # breakpoint()
