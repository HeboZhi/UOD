...
class AFF(nn.Module):
    """
    MSCA-AFF
    Multi-Scale Spatial-Channel Adaptive Feature Fusion

    设计思想：
    1. 多尺度局部分支：建模不同感受野下的局部结构
    2. 全局通道分支：利用 AvgPool + MaxPool 获取全局上下文
    3. 空间选择分支：显式建模“哪里该融合”
    """
    def __init__(self, channels=64, r=4):
        super(AFF, self).__init__()
        inter_channels = max(channels // r, 8)
        ...
        self.local_3x3 = nn.Conv2d(
            ...
            groups=inter_channels, bias=False
        )
        self.local_5x5 = nn.Conv2d(
            inter_channels, inter_channels,
            kernel_size=5, padding=2,
            groups=inter_channels, bias=False
        )
        self.local_7x7 = nn.Conv2d(
            inter_channels, inter_channels,
        ...
        self.local_fuse = nn.Sequential(
            nn.Conv2d(inter_channels * 3, channels, kernel_size=1, bias=False),
    ...
    def forward(self, x, residual):
        ...
        # 1) 局部多尺度响应
        # =========================
        local_feat = self.local_reduce(xa)

        feat_3 = self.local_3x3(local_feat)
        feat_5 = self.local_5x5(local_feat)
        feat_7 = self.local_7x7(local_feat)

        xl = torch.cat([feat_3, feat_5, feat_7], dim=1)
        xl = self.local_fuse(xl)

        # =========================
        # 2) 全局通道响应
        # =========================
        xg_avg = self.channel_mlp(self.avg_pool(xa))
        xg_max = self.channel_mlp(self.max_pool(xa))
        xg = xg_avg + xg_max

        # =========================
        # 3) 空间选择响应
        # =========================
        xs = self.spatial_att(xa)

        # =========================
        # 4) 最终融合权重
        # =========================
        wei = self.sigmoid(xl + xg + xs)
