...
def OOD_detection_postprocessing(input_im,
    ...
    if not pretest:
        keep = remove_by_scores(outputs, 0.531, retain_larger=True)
        update(outputs, keep)
        ...
        similarity = torch.cosine_similarity(outputs['complete_feat'].unsqueeze(1),
                                             outputs['complete_feat'].unsqueeze(0), dim=-1)
