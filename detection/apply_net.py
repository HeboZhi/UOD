...
def main(args):
    ...
    if not args.eval_only:
        ...
        if args.pretest:
            with open(os.path.join(big_inference_output_dir, '{}_instances_results_pretest.json'.format(dataname)),
                      'w') as fp:
                json.dump(final_output_list_idood, fp, indent=4, separators=(',', ': '))
        else:

            with open(os.path.join(big_inference_output_dir, '{}_instances_results_idood.json'.format(dataname)),
                      'w') as fp:
                json.dump(final_output_list_idood, fp, indent=4, separators=(',', ': '))
