...
class ObjectnessHead(nn.Module):
    """
    [新增] 前景背景二分类头 (Objectness Head)
    输入: Box Head 提取的特征 (通常是 FC 层的输出)
    输出: Logits (1维)，表示是物体的概率 (Objectness)
    """

    def __init__(self, input_dim, hidden_dim=256):
        super().__init__()
        # 两层 MLP 结构，用于从共享特征中解耦出 Objectness 信息
    ...
    def forward(self, x):
        # x shape: [Batch_Size * Num_Proposals, Input_Dim]
        x = F.relu(self.fc1(x))
        logits = self.fc2(x)
        print("我在用分类头",flush=True)
        return logits
...
class ProbabilisticROIHeads(StandardROIHeads):
    ...
    def __init__(self, cfg, input_shape):
        ...
        # ================== [新增代码 START] ==================
        # 初始化 Objectness Head
        # 我们假设 box_head 输出的是 FC 特征 (例如 1024 维)
        # 如果你使用的是 FastRCNNConvFCHead，维度通常在 cfg.MODEL.ROI_BOX_HEAD.FC_DIM
        input_dim = cfg.MODEL.ROI_BOX_HEAD.FC_DIM

        # 实例化模块
        self.objectness_head = ObjectnessHead(input_dim=input_dim)

        # 定义二分类损失函数 (BCEWithLogitsLoss 自带 Sigmoid，数值更稳定)
        self.objectness_loss_func = nn.BCEWithLogitsLoss()
        # ================== [新增代码 END] ==================
    ...
    def _forward_box(
        ...
        # 1. ROI Pooling (从特征图中抠图)
        box_features = self.box_pooler(
            features, [x.proposal_boxes for x in proposals])

        # 2. Box Head (FC Layers) 特征提取
        # box_features 在这里被展平并经过 FC 层 (变成 1024 维向量)
        ...
        # ================== [新增代码 START] ==================
        # 4. Objectness 预测 (二分类：是物体 vs 背景)
        # 使用共享的 box_features
        objectness_logits = self.objectness_head(box_features)
        # ================== [新增代码 END] ==================

        del box_features

        if self.produce_raw_output:
            # 如果需要，你也可以把 objectness_logits 加进返回值
            return predictions
        ...
        if self.training:
            ...
            # ================== [新增代码 START] ==================
            # 5. 计算 Objectness Loss 并加入总 Loss
            obj_loss = self._compute_objectness_loss(objectness_logits, proposals)

            # 权重设为 0.5 (可以根据实验调整)，防止主导分类 Loss
            losses.update({"loss_objectness": obj_loss * 0.5})
            # ================== [新增代码 END] ==================
            ...
            # ================== [新增代码 START] ==================
            # 在推理阶段，我们将 Objectness 分数保存到结果中
            # 这对于你的 AUD 模型后续的 "BAD 策略" (后处理) 非常有用

            # 1. 计算 sigmoid 得到 0~1 的分数
            objectness_scores = torch.sigmoid(objectness_logits).view(-1)

            # 2. 我们需要把这些分数分配给过滤后的 pred_instances
            # 注意：box_predictor.inference 内部做了 NMS 和 TopK 过滤，
            # 所以我们需要确保 objectness_scores 也能对应上。
            # 但 standard inference 比较复杂，通常我们只能拿到 NMS 之后的结果。
            #
            # 简单做法：我们先不修改 inference 的过滤逻辑，
            # 而是把 objectness 作为一个单独的属性附带在原始预测上，
            # 但由于 fast_rcnn_inference 会丢弃索引，这里最简单的做法是：
            # 如果你想利用这个分数，需要在 box_predictor.inference 内部改，
            # 或者这里暂不处理，仅用于训练特征增强。
            #
            # 既然你是做 AUD，通常需要保留这个分数。但为了不破坏现有逻辑，
            # 我建议暂时不在 pred_instances 里强行插入，除非你修改 fast_rcnn_inference。
            # ================== [新增代码 END] ==================

            return pred_instances

    def _compute_objectness_loss(self, logits, proposals):
        """
        [新增] 计算前景背景二分类损失
        """
        # 1. 拼接所有图片的 GT 类别
        # 在 Detectron2 的 Proposal 中，gt_classes 存储了每个 Proposal 对应的真实类别
        # 背景类的索引通常等于 num_classes
        gt_classes = cat([p.gt_classes for p in proposals], dim=0)
        print("二分类损失在使用")
        # 获取背景类的索引 (从 box_predictor 获取)
        # cls_score 输出维度是 num_classes + 1 (背景)
        bg_class_ind = self.box_predictor.cls_score.out_features - 1

        # 2. 生成二分类标签
        # 逻辑：只要 gt_classes 不是背景索引，它就是前景 (Label=1)，否则是背景 (Label=0)
        # .float() 将布尔值转换为 0.0/1.0
        target_labels = (gt_classes != bg_class_ind).float().unsqueeze(1)

        # 3. 计算 BCE With Logits Loss
        loss = self.objectness_loss_func(logits, target_labels)
        return loss
