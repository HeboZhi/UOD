...
class ROIHeadsLogisticGMMNewLimitFPEnergy(ROIHeadsLogisticGMMNew):
    ...
    def sample_localboxes_and_completeboxes_t2(self, proposals, targets):
        ...
        for i, img in enumerate(proposals):
            ...
            pos_x_proposals, pos_y_proposals, pos_bool_list = self.export_pos_box(proposal_boxes, iou_with_gt,
                                                                                  match_gtid, gt_num, e2)
    ...
    def loss_local_complete_t2(self, goc_samples, features):
        ...
        MSELoss = nn.MSELoss()
        loss_pos2 = MSELoss(complete_scores_x,
                            torch.ones_like(complete_scores_x))  # F.relu(-1 * torch.log(complete_scores_x + 0.0001))
        loss_pos3 = MSELoss(complete_scores_y,
                            torch.ones_like(complete_scores_y))  # F.relu(-1 * torch.log(complete_scores_y + 0.0001))
        ...
        loss_neg = F.relu(
            complete_scores_neg + 1e-3 - minscores)  # MSELoss(complete_scores_neg, torch.zeros_like(complete_scores_neg))#F.relu(complete_scores_neg)
        ...
        print("{}  {}   {}   {}   {}".format(minscores, loss_pos1.mean(), loss_pos2.mean(), loss_pos3.mean(),
                                             loss_neg.mean()))
        return loss
...
class ROIHeadsLogisticGMMNewLimitFPEnergy_IouLoss(ROIHeadsLogisticGMMNew):
    ...
    def loss_local_complete_t2(self, goc_samples, features, targets):
        ...
        minscores = 0.5  # if tmp.shape[0] == 0 else tmp.min().item()
        loss_neg = F.relu(
            complete_scores_neg + 1e-3 - iou_with_gt_list_neg)  # MSELoss(complete_scores_neg, torch.zeros_like(complete_scores_neg))#F.relu(complete_scores_neg)
